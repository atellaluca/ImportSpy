...
from importspy.utils.die_utils import ModuleUtils
...
def mock_import_no_plugin(monkeypatch):
    ...
    monkeypatch.setattr(ModuleUtils(), 'load_module', mock_load_module)

    def mock_unload_module(info_module):
        logger.debug(f"unload module: {info_module}")
    
    monkeypatch.setattr(ModuleUtils(), 'unload_module', mock_unload_module)