import pytest
from importspy.models import Module, Variable
...
class TestModuleValidator:
    ...
    @pytest.fixture
    def data_2(self):
        return [Module(filename="package.py", version="0.1.0")]

    @pytest.fixture
    def version_02_module(self):
        return [Module(filename="package.py", version="0.2.0")]
    ...
    def variables_setter(self, data_2: List[Module]):
        data_2[0].variables = [
            Variable(name="attempt", value=4),
            Variable(name="msg", value="Hi!"),
        ]
    ...
    def data_3(self):
        return [
            Module(
                filename="package.py",
                version="0.1.0",
                variables=[Variable(name="attempt", value=4)],
            )
        ]
    ...
    def test_module_match(
        ...
        data_1: List[Module],
        data_2: List[Module],
        module_contract: ModuleContractViolation,
    ):
    ...
    def test_module_mismatch(
        self, data_2: List[Module], module_contract: ModuleContractViolation
    ):
        assert self.validator.validate(None, data_2[0], module_contract) is None

    @pytest.mark.usefixtures("version_unsetter")
    def test_module_filename_mismatch(
        self,
        data_1: List[Module],
        data_2: List[Module],
        module_contract: ModuleContractViolation,
    ):
        module_contract.bundle[Errors.KEY_FILE_NAME] = data_1[0].filename
        with pytest.raises(
            ValueError,
            match=re.escape(
                module_contract.mismatch_error_handler(
                    data_1[0].filename, data_2[0].filename, Errors.ENTITY_MESSAGES
                )
            ),
        ):
    ...
    @pytest.mark.usefixtures("version_unsetter")
    def test_module_missing_version(
        self,
        data_3: List[Module],
        data_2: List[Module],
        module_contract: ModuleContractViolation,
    ):
        module_contract.bundle[Errors.KEY_FILE_NAME] = data_3[0].filename
        with pytest.raises(
            ValueError,
            match=re.escape(
                module_contract.mismatch_error_handler(
                    data_3[0].version, data_2[0].version, Errors.ENTITY_MESSAGES
                )
            ),
        ):
            self.validator.validate(data_3, data_2[0], module_contract)

    def test_module_version_mismatch(self, version_02_module, data_2, module_contract):
        module_contract.bundle[Errors.KEY_FILE_NAME] = version_02_module[0].filename
        with pytest.raises(
            ValueError,
            match=re.escape(
                module_contract.mismatch_error_handler(
                    version_02_module[0].version,
                    data_2[0].version,
                    Errors.ENTITY_MESSAGES,
                )
            ),
        ):
            self.validator.validate(version_02_module, data_2[0], module_contract)
