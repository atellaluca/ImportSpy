...
class Constants:
    ...
    Attributes:
    -----------
    KNOWN_ARCHITECTURES (list[str]): List of known architectures supported by ImportSpy.
    SUPPORTED_OS (list[str]): List of operating systems supported by ImportSpy.
    SUPPORTED_PYTHON_IMPLEMENTATION (list[str]): Supported Python implementations.
    SUPPORTED_CLASS_ATTRIBUTE_TYPES (list[str]): Types of class attributes recognized by ImportSpy.
    SUPPORTED_ANNOTATIONS (list[str]): Supported annotations for functions, arguments, and variables.
    DIE_ENVIRONMENT (str): Default environment name for ImportSpy.
    NAME (str): Key representing the name of an entity.
    VALUE (str): Key representing the value of an entity.
    CLASS_TYPE (str): Type identifier for class-level attributes.
    INSTANCE_TYPE (str): Type identifier for instance-level attributes.
